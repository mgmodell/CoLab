Feature: Creating Inside Assessments
  ...
  These weeklies are simple Sun - Sat. with no wrap-around.
  ...
  Scenario: if the set_up_weekly process runs on three successive weeks when assessments are open, four weeklies will be attached
    Given the assessment has been activated
    Given today is "next Monday"
    Given that the system's set_up_weeklies process runs
    Given that the system's set_up_weeklies process runs
