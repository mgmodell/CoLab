...
import {
  IAssignment,
  IRubricRowFeedback,
  ISubmissionCondensed
} from "./AssignmentViewer";
...
export default function FeedbackVisualization(props: Props) {
  ...
  const maxId = props.assignment.rubric.criteria.reduce((maximum, next) => {
    return Math.max(maximum, next.id);
  }, 0);
  const minId = props.assignment.rubric.criteria.reduce((maximum, next) => {
  ...
  useEffect(() => {
    const localProgressData = props.assignment.submissions
      .map((submission: ISubmissionCondensed) => {
        const feedbacks = {};
        if (submission.rubric_row_feedbacks.length > 0) {
          submission.rubric_row_feedbacks.forEach(
            (lFeedback: IRubricRowFeedback) => {
  ...
  return (
    <>
      ...
      >
        ...
        />
        <AnimatedAxis axisClassName="y-axis" orientation="left" label="Score" />
        ...
        <Tooltip
          ...
          renderTooltip={({ tooltipData, colorScale }) => {
            const content = Object.values(
              tooltipData.nearestDatum.datum.feedbacks
            ).map((feedback, index) => {
              return (
        ...
        {chartType === CHART_TYPES.LINES
          ? props.assignment.rubric.criteria.map(
              (criterium: ICriteria, index) => {
