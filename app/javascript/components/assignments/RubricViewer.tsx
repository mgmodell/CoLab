  ...
  const evaluation =
    props.rubric !== undefined ? (
      <Grid container columns={70}>
        ...
        <Grid xs={20}>
          <Typography>{props.rubric.name}</Typography>
        </Grid>
        <Grid xs={10}>
          <Typography variant="h6">{t("status.rubric_version")}:</Typography>
        </Grid>
        <Grid xs={20}>
          <Typography>{props.rubric.version}</Typography>
        </Grid>
          ...
          .map(criterium => {
            ...
            return (
              <Grid container key={criterium.id} columns={70}>
                ...
                <Grid xs={10}>{criterium.description}</Grid>
                <Grid xs={span}>{t("status.rubric_minimum")}</Grid>
                <Grid />
                {renderedLevels}
              </Grid>
            );
