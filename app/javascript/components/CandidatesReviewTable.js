    ...
    this.state = {
      ...
      columns: 
            ...
            },
            {
              ...
              visible: true,
              sortable: true,
              render_func: (c)=>{ return c.definition }
            },
            {
              width: 200,
              flexGrow: 1.0,
              ...
              visible: true,
              sortable: true,
              render_func: (c)=>{return this.feedbackRender(c)}
            },
            {
              width: 200,
              flexGrow: 1.0,
              ...
              visible: true,
              sortable: true,
              render_func: (c)=>{ return this.conceptRender(c) }
            },
          ]
    }
...
//      open( data.next_stop, '_self' )
  ...
  conceptRender = function( c ){
    const label = 'Concept'
    return(
    <RemoteAutosuggest inputLabel={label}
                            itemId={c.id}
  ...
  feedbackRender = function( c ){
    ...
    return(
    <SelectFrom options={this.state.feedback_opts}
                            selectLbl={label}
                            itemId={c.id}
                            selectedValue={id}
  ...
  render() {
    const { columns, candidates, candidates_raw, rowsPerPage, page } = this.state;
    return (
        ...
        <Table>
          <TableHead>
            ...
            {columns.map( cell => {
              if( cell.visible ){
              return(
                <TableCell key={'h_' + cell.dataKey}>{cell.label}</TableCell>
              )
              }
            })}
            </TableRow>
