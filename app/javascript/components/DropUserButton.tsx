...
export default function DropUserButton(props) {
  ...
  const copyDialog = (
    ...
    >
      <DialogTitle id="alert-delete-confirm">{"Confirm dropping student from course"}</DialogTitle>
      <DialogContent>
        <DialogContentText id="alert-delete-confirm-description">
          Are you sure you want to drop this user from this course? This can be reversed later, but it may result in lost data and confusion on the part of the student.
        </DialogContentText>
      </DialogContent>
      <DialogActions>
  ...
  return (
    <React.Fragment>
      <Tooltip key='delete-forever' title={'Drop Student'}>
        <IconButton aria-label={'Drop Student'}
          onClick={(event)=>{
            setShowDialog( true );
          }}
          >
