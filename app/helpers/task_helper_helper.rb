  ...
  def task_link(task:, anonymize:, user:)
    if task.class == BingoGame
      ...
      o_string = if task.awaiting_review?
                   link_to "Review: #{task.get_name(anonymize)}",
                           review_bingo_candidates_path(task)
                 elsif task.is_open?
                   link_to task.get_name(anonymize),
                           edit_candidate_list_path(candidate_list)
                 elsif task.reviewed
                   link_to task.get_name(anonymize), candidate_list_path(
                     candidate_list
                   )
                 else
                   "BingoGame: #{task.get_name(anonymize)}"
                 end
    ...
    #{task.project.get_name(anonymize)})</small>"
      ...
      o_string = if reaction.behavior.present?
                   ...
                   link_to task.get_name(anonymize), next_experience_path(
