class User < ActiveRecord::Base
  ...
  def waiting_tasks
    ...
    available_rosters.each do |roster|
      waiting_tasks.concat roster.course.experiences.where( 'experiences.end_date >= ? AND experiences.start_date <= ? AND experiences.active = ?', DateTime.current, DateTime.current, true).to_a
    end
